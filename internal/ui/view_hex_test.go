...
func TestPointerWordStart(t *testing.T) {
	...
	if got := m.pointerWordStart(0x1000, 0x40); got != 0x40 {
		t.Fatalf("aligned cursor = %d, want 0x40", got)
	}
	if got := m.pointerWordStart(0x1003, 0x43); got != 0x40 {
...
func TestReadPointerEndianness(t *testing.T) {
	...
	if v, ok := be.readPointer(data, 0); !ok || v != 0x0102030405060708 {
		t.Fatalf("big-endian pointer = %#x (ok=%v)", v, ok)
	}
	if _, ok := le.readPointer(data, 4); ok {
